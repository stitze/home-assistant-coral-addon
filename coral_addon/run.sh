...
# Automatische Versionierung (Teil deines Hooks)
...
if [ -n "$CONFIG_FILE" ]; then
    ...
    NEW_MINOR=$minor
    NEW_MAJOR=$major
    if [ "$NEW_PATCH" -gt 9 ]; then NEW_PATCH=0; NEW_MINOR=$((minor + 1)); fi
    if [ "$NEW_MINOR" -gt 9 ]; then NEW_MINOR=0; NEW_MAJOR=$((major + 1)); fi
...
try:
    # Laden ueber den System-Namen nach ldconfig
